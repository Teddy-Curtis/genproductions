...
mH, mA, mHch, process, base_dir = sys.argv[1], sys.argv[2], str(sys.argv[3]), str(sys.argv[4]), str(sys.argv[5])
...
# Now run this
cmd = f"python2 {MG_dir}/bin/mg5_aMC find_h3_decay_{process}_mH{mH}_mA{mA}_mHch{mHch}.txt"
...
# Now run this
cmd = f"python2 {MG_dir}/bin/mg5_aMC find_hch_decay_{process}_mH{mH}_mA{mA}_mHch{mHch}.txt"
status, out = subprocess.getstatusoutput(cmd)
print(out)
...
# cmd = f"cp -f {process}_mH{mH}_mA{mA}_mHch{mHch}_customizecards.dat {base_dir}/{process}_mH{mH}_mA{mA}_mHch{mHch}/{process}_mH{mH}_mA{mA}_mHch{mHch}_customizecards.dat"
# status, out = subprocess.getstatusoutput(cmd)

cmd = f"rm -r tmp/base_{process}_mH{mH}_mA{mA}_mHch{mHch}"
status, out = subprocess.getstatusoutput(cmd)
print(out)