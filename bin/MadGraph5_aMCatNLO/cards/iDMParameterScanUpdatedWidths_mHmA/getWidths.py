...
with open(f'{process}_mH{mH}_mA{mA}_customizecards.dat', 'r') as f:
    set_commands = f.read()
print(set_commands)
...
with open(f'{process}_mH{mH}_mA{mA}_customizecards.dat', 'w') as f:
    f.write(set_commands)


# Now I copy it back to afs 
cmd = f"cp -f {process}_mH{mH}_mA{mA}_customizecards.dat {base_dir}/{process}/{process}_mH{mH}_mA{mA}/{process}_mH{mH}_mA{mA}_customizecards.dat"
status, out = subprocess.getstatusoutput(cmd)