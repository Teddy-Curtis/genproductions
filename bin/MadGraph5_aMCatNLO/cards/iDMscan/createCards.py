'''
Here we make the cards for a BP scan, but we will only do this for the BPs 
8, 10, 12, 13, 14, 18, 19, 20, 21, 24
I do a scan of lam2 ranging from -1.5 to 1.5 as the biggest value of lam2 is for
BP 20 with a value ~2.6/2. I take 10 values in this range.
NOTE: whilst the value is given by 2.6, in the param_card it is given by the value
divided by 2. So the actual loop is from -3 to 3
'''
import os
import numpy as np
...
for BP_num, params in enumerate(BP_params):
    BP_num += 1
    if BP_num in BPs_scan:
        print(f'BP = {BP_num}')
        for i, lam2 in enumerate(lam2_vals):
            params[3] = lam2
            print(f'Value of lam2 = {lam2}')
            run_name = f'{run_prefix}_BP{BP_num}_lam{i}'
